...
## 6) Common mistake: opening endpoint PHP files directly
If you open URLs like:
- `http://localhost/tyreswift/tyreswift-backend/api/accept_request.php`

in a browser address bar, the request method is **GET**. Endpoints such as `accept_request` require **POST**, so you will correctly see:
- `{"success":false,"message":"Method not allowed. Use POST."}`

Use one of these instead:
- Postman (set method to `POST`, body `raw JSON`)
- cURL commands in section 4

Compatibility note:
- The router now also accepts legacy paths like `/api/accept_request.php`, but method rules still apply.
